...
cd emacs-29.4/
./autogen.sh &
./configure --prefix="/usr/local/emacs/29.4" --without-compress-install --with-native-compilation --with-json --with-mailutils --with-tree-sitter CC=gcc-12 &
...
ln -s /usr/local/emacs/current /usr/local/emacs/29.4
